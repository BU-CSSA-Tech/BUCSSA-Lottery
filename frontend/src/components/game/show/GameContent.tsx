    ...
    >
      ...
      {gameState && (
        <motion.div
          ...
          className={
            isQuestionActive
              ? "flex shrink-0 flex-wrap justify-center gap-8"
              : "flex flex-wrap justify-center gap-12 mb-12"
          }
        >
      ...
      {gameState.currentQuestion && gameState.status === "playing" && (
        <motion.div
          ...
          className="theme-panel flex w-full flex-col items-center gap-12 px-8 py-8"
        >
          <h2 className="w-full shrink-0 text-center font-light leading-tight text-gray-800 text-[clamp(2.5rem,6.5vh,4.5rem)]">
            {gameState.currentQuestion.question}
          </h2>

          <div className="grid w-full shrink-0 grid-cols-2 items-stretch gap-6 md:gap-10">
            <div
              className={
                isNailong
                  ? "theme-show-option flex min-w-0 items-center justify-center rounded-2xl px-6 py-[2vh]"
                  : "flex min-w-0 items-center justify-center rounded-2xl border border-green-200/60 bg-green-50/90 px-6 py-[2vh]"
              }
            >
              <div className="flex w-full items-center justify-center gap-4">
                {isNailong ? (
                  ...
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-green-100 md:h-20 md:w-20">
                    <p className="font-bold text-green-600 !leading-none !text-[clamp(2rem,4.5vh,3.5rem)]">A</p>
                  </div>
                )}
                <p className="min-w-0 text-center font-medium leading-snug text-gray-800 text-[clamp(1.875rem,4vh,3rem)]">
            ...
            <div
              className={
                isNailong
                  ? "theme-show-option flex min-w-0 items-center justify-center rounded-2xl px-6 py-[2vh]"
                  : "flex min-w-0 items-center justify-center rounded-2xl border border-red-200/60 bg-red-50/90 px-6 py-[2vh]"
              }
            >
              <div className="flex w-full items-center justify-center gap-4">
                {isNailong ? (
                  ...
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-red-100 md:h-20 md:w-20">
          ...
          <div className="flex w-full shrink-0 items-center justify-center text-center">
            <div
              className={`show-countdown-size flex items-center justify-center overflow-hidden rounded-full font-bold tabular-nums ${
                frontendTimeLeft <= 10 ? "show-countdown-urgent animate-pulse" : ""
              }`}
            >
      ...
      {gameState.status === "waiting" && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className={`theme-panel shadow-red-down flex w-full max-w-4xl items-center justify-center ${
            isNailong ? "flex-row-reverse gap-6 px-6 py-8" : "gap-16 px-16 py-16"
          }`}
        >
          <div className="flex flex-col items-center gap-5 justify-center">
            <div className="flex items-center gap-4">
              ...
              <h3 className="text-4xl font-bold text-gray-800 tracking-wider">
                游戏准备中
              </h3>
            </div>
          ...
          {pack.waitingDecor && (
            <div className="flex-shrink-0 w-52 h-52 md:w-64 md:h-64 flex items-center justify-center">
              <Image src={pack.waitingDecor} alt="等待抽奖的小狗" width={256} height={256} className="h-full w-full object-contain" />
            </div>
          )}
        </motion.div>
      )}
